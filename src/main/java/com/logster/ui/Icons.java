package com.logster.ui;

import com.formdev.flatlaf.extras.FlatSVGIcon;

public class Icons {

    public static final FlatSVGIcon searchIcon = new FlatSVGIcon("icons/search.svg",16,16);
    public static final FlatSVGIcon cancelIcon = new FlatSVGIcon("icons/cancel.svg",16,16);
    public static final FlatSVGIcon closeIcon = new FlatSVGIcon("icons/close.svg",16,16);
    public static final FlatSVGIcon labIcon = new FlatSVGIcon("icons/lab.svg",16,16);
    public static final FlatSVGIcon limitIcon = new FlatSVGIcon("icons/limit.svg",16,16);
    public static final FlatSVGIcon previewIcon = new FlatSVGIcon("icons/preview.svg",16,16);
    public static final FlatSVGIcon searchingIcon = new FlatSVGIcon("icons/searching.svg",16,16);
    public static final FlatSVGIcon thumbsUpIcon = new FlatSVGIcon("icons/thumbsUp.svg",16,16);
    public static final FlatSVGIcon openFolderIcon = new FlatSVGIcon("icons/openFolder.svg",16,16);
    public static final FlatSVGIcon stopIcon = new FlatSVGIcon("icons/stop.svg",16,16);
    public static final FlatSVGIcon extensionIcon = new FlatSVGIcon("icons/extension.svg",16,16);
    public static final FlatSVGIcon dateIcon = new FlatSVGIcon("icons/date.svg",16,16);
}
